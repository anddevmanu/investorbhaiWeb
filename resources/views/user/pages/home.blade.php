                    ...
                    success: function(data) {
                        if (data.data.length > 0) {
                            ...
                            $.each(data.data, function(index, post) {
                                var formattedDate = new Date(post.created_at).toLocaleDateString('en-US', {
                                    year: 'numeric',
                                    month: 'long',
                                    day: 'numeric'
                                });
                                posts += '<div class="bg-white shadow-md rounded-lg mb-4">';
                                posts += '<div class="px-4 py-2 bg-gray-100 border-b">';
                                posts += '<p class="text-lg font-semibold m-0">';
                                posts += '<a href="/questions/' + post.slug + '" class="text-blue-600 hover:underline">' + post.title + '</a>';
                                posts += '</p>';
                                posts += '@if (auth()->check() && (auth()->user()->role == "editor" || auth()->user()->role == "admin"))';
                                posts += '<a href="/user/questions/edit/' + post.id + '" class="text-white bg-green-600 hover:bg-green-500 px-2 py-1 rounded-md float-right"><i class="fa fa-edit"></i></a>';
                                posts += '@endif';
                                posts += '</div>';
                                posts += '<div class="px-4 py-2">';
                                posts += '<span class="bg-green-500 text-white text-sm rounded-full px-2 py-1 inline-flex items-center mr-2">';
                                posts += '<i class="fa fa-check mr-1"></i>';
                                posts += '</span>';
                                posts += '<span class="bg-blue-500 text-white text-sm rounded-full px-2 py-1 inline-flex items-center mr-2">';
                                posts += '<i class="fa fa-eye mr-1"></i> ' + post.views;
                                posts += '</span>';
                                posts += '<span class="bg-blue-500 text-white text-sm rounded-full px-2 py-1 inline-flex items-center mr-2">';
                                posts += '<i class="fa fa-thumbs-up mr-1"></i> ' + post.likes;
                                posts += '</span>';
                                if (post.tags) {
                                    $.each(post.tags.split(','), function(index, tag) {
                                        posts += '<span class="bg-gray-200 text-gray-700 text-sm rounded-full px-2 py-1 inline-flex items-center mr-2">';
                                        posts += $.trim(tag);
                                        posts += '</span>';
                                    });
                                }
                                posts += '<span class="bg-yellow-400 text-gray-800 text-sm rounded-full px-2 py-1 float-right">';
                                posts += formattedDate;
                                posts += '</span>';
                                posts += '</div>';
                                posts += '</div>';
